...
# MASTER PEOPLE SYNC (Name is source-of-truth in Master)

# ============================================================

def sync_person_row_fields_prefer_name(row: Dict[str, Any], maps: dict) -> Dict[str, Any]:
    ...
    allowed = set([n.lower() for n in (maps.get("names_sorted") or []) if str(n).strip()])

 

    def _sync(name_key: str, id_key: str) -> None:

        nm = _norm(out.get(name_key, "")) or ""

        emp_id = _norm(out.get(id_key, "")) or ""

 

        # invalid name -> NULL everything for this pair
        ...
        # prefer name (dropdown)

        if nm:

            mapped_id = name_to_id.get(nm.lower())

            out[id_key] = mapped_id or None

            out[name_key] = nm  # keep user's selection text

            return

 

        # fallback: if name empty and ID exists -> derive name

        if emp_id and emp_id in id_to_name:
    ...
    _sync("project_manager_name", "project_manager_id")

    _sync("support_lead_name", "support_lead_id")

    _sync("responsible_analyst_name", "responsible_analyst_id")

 

    # Region derived from support lead (ID preferred; else from name)

    sl_id = _norm(out.get("support_lead_id", "")) or ""

    sl_name = _norm(out.get("support_lead_name", "")) or ""

 

    if sl_id:

        out["region"] = id_to_region.get(sl_id) or None

    elif sl_name:

        mapped = name_to_id.get(sl_name.lower())

        out["support_lead_id"] = mapped or None

        out["region"] = id_to_region.get(mapped) if mapped else None
...
def apply_master_autofill(df_display: pd.DataFrame, maps: dict) -> pd.DataFrame:
    ...
    for i, r in out.iterrows():
        ...
        row = {
            ...
            "project_manager_id": None,  # always derived
        ...
        row = sync_person_row_fields_prefer_name(row, maps)
...
def get_master_df() -> pd.DataFrame:
    ...
    df = fetch_df(f"SELECT {MASTER_SELECT_COLUMNS} FROM {T_MASTER} ORDER BY jira_id")
    ...
    # DEFAULT INVESTIGATION STATUS (UI + downstream logic): blank/NULL -> Not Started

    if "qc_investigation_status" in df.columns:

        s = df["qc_investigation_status"].fillna("").astype(str).str.strip()

        s.loc[s.str.lower().isin(["nan", "none"])] = ""

        df["qc_investigation_status"] = s.replace({"": "Not Started"})
    ...
    # sanitize invalid stored names -> blank (master view)

    maps = get_employees_maps()

    allowed = set([n.lower() for n in (maps.get("names_sorted") or []) if str(n).strip()])

    for col in ["project_manager_name", "support_lead_name", "responsible_analyst_name"]:

        if col in df.columns:

            s = df[col].fillna("").astype(str).str.strip()

            mask_invalid = (s != "") & (~s.str.lower().isin(allowed))

            df.loc[mask_invalid, col] = ""
...
def update_master_fields_many(payload_rows: List[dict]) -> None:
    ...
    for col in candidate_cols:
        ...
        if col == "jira_summary":
            ...
            set_clauses.append("jira_summary = :jira_summary")
...
def build_master_grid(
    ...
    if "JIRA Title" in df_display.columns:
    ...
    # People: Names are dropdowns, IDs+Region locked

    for col in ["Project Manager Name", "Support Lead Name", "Responsible Analyst Name"]:
    ...
    for col in ["Project Manager ID", "Support Lead ID", "Responsible Analyst ID", "Region"]:
    ...
    # Clipboard / Excel-like behaviour
    ...
    # Some builds need this to allow Ctrl+V to be captured
    ...
    enable_enterprise_modules=True  # <- replaces update_mode
...
def show_master_view() -> None:
    ...
    with k3:
        ...
        st.markdown(
            ...
            kpi_card_html("In-Progress", int(inv_counts.get("In-Progress", 0)), "kpi-yellow"),
    ...
    with o2:
        ...
        st.markdown(
            ...
            kpi_card_html("Generally", int(overall_counts.get("Generally Conforms", 0)), "kpi-lgreen"),
    ...
    # FIX: keep a DB baseline snapshot separate from the working grid,

    # otherwise Save compares "working" vs "working" after reruns and shows "No changes".
    ...
    baseline_state_key = "master_grid_baseline"   # NEW

 

    current_view_key = "|".join(sorted(df_show["JIRA ID"].astype(str).tolist()))

 

    if st.session_state.get(grid_key_state) != current_view_key:

        st.session_state[baseline_state_key] = df_show.copy(deep=True)  # baseline from DB

        st.session_state[grid_state_key] = df_show.copy(deep=True)      # editable working copy
    ...
    baseline_df = st.session_state.get(baseline_state_key, df_show.copy(deep=True))

    if not isinstance(baseline_df, pd.DataFrame):

        baseline_df = df_show.copy(deep=True)

 

    df_before_edit = baseline_df.copy(deep=True)  # IMPORTANT: baseline (DB snapshot), not working

 

    maps = get_employees_maps()

    employee_names = [""] + (maps.get("names_sorted") or [])

 

    edited_df = build_master_grid(grid_input, display_to_internal, employee_names)
    ...
    # If sync changed anything -> update working DF and rerun (IDs/Region)
    ...
    if not _df_equal_loose(edited_sanitized, edited_df):
        ...
        st.info("People fields were auto-synced (Name→ID, Region from Support Lead) and invalid names cleared.")
    ...
    with csave1:
        ...
        if st.button("Save", type="primary", use_container_width=True):
            ...
            for _, r in changed.iterrows():
                ...
                # People: derive from Names (do NOT trust IDs from grid)
                ...
                # Default investigation status on save: blank/NULL -> Not Started

                inv_status = _norm(r.get("QC Investigation Status", "")) or "Not Started"
                ...
                row = {
                    ...
                    "jira_summary": _norm(r.get("JIRA Title", "")) or None,  # keep title read-only

                    "qc_overall_status": _norm(r.get("QC Overall Status", "")) or None,

                    "qc_investigation_status": inv_status,
                    ...
                    "project_manager_id": people.get("project_manager_id"),

                    "project_manager_name": people.get("project_manager_name"),

                    "support_lead_id": people.get("support_lead_id"),

                    "support_lead_name": people.get("support_lead_name"),

                    "responsible_analyst_id": people.get("responsible_analyst_id"),

                    "responsible_analyst_name": people.get("responsible_analyst_name"),

                    "region": people.get("region"),
            ...
            # IMPORTANT: reset baseline + working so next rerun re-pulls DB and diff works correctly

            st.session_state.pop(baseline_state_key, None)

            st.session_state.pop(grid_state_key, None)

            st.session_state.pop(grid_key_state, None)
...
def get_master_one(jira_id: str) -> Optional[pd.Series]:
    ...
    df = fetch_df(
        ...
        f"SELECT {MASTER_SELECT_COLUMNS} FROM {T_MASTER} WHERE jira_id = :jira_id",
...
def show_detail_view(jira_id: str) -> None:
    ...
    # sanitize invalid names on load

    for k in ["project_manager_name", "support_lead_name", "responsible_analyst_name"]:

        v = _norm(st.session_state.get(k))
    ...
    with g3:
        ...
        inv_val = master.get("qc_investigation_status") or "Not Started"
    ...
    st.markdown(
        ...
        '<div class="small-muted">Dates accept empty value for NULL. Format: <b>YYYY-MM-DD</b>.</div>',
    ...
    remediation_meeting_date_s = d3.text_input(

        "Remediation Meeting Date", value=date_str(safe_date(master.get("remediation_meeting_date")))
    ...
    with csave1:
        ...
        if st.button("Save", type="primary", use_container_width=True):
            ...
            payload = {
                ...
                "qc_investigation_status": (qc_investigation_status or "Not Started"),
